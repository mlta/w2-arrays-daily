...
function union() {
  // Iterate over the "first" and "second" arrays.
  let same = []
  for (var i = 0; i < first.length; i++) {
    for (var j = 0; j < second.length; j++) {
      let firstArrayValue = first[i]
      let secondArrayValue = second[j]
      // Compare the values for both arrays.
      if (firstArrayValue === secondArrayValue) {
        // If the arrays values are the same, then store that value in the "same" array.
        same.push(firstArrayValue)
      }
    }
  }
  // Return "same".
